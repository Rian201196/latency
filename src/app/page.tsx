...
import BarChart from '../components/chart/acc';
...
export default function HomePage() {
  ...
  useEffect(() => {
    const fetchSensors = async () => {
      try {
        const response = await fetch('/api/sensors'); // Ganti dengan endpoint API Anda
  ...
  return (
    <div className="min-h-screen bg-[rgb(212,218,220)] p-2 sm:p-4">
      <div className="grid h-[calc(100vh-1rem)] grid-cols-1 sm:grid-cols-12 sm:grid-rows-9 gap-2 sm:gap-4">
        ...
        <div id="cetak" className="sm:col-span-3 sm:row-span-1 rounded-lg relative">
          <Image
            src="/cetak.png"
            alt="Cetak Image"
        ...
        <div id="aktif" className="sm:col-span-3 sm:row-span-1 rounded-lg bg-[#d9fce4] opacity-70 p-2">
        ...
        <div id="mati" className="sm:col-span-3 sm:row-span-1 rounded-lg bg-[#feeeee] opacity-70 p-2">
          <div className="grid grid-cols-4 h-full items-center gap-2">
            <div className="icon col-span-1 text-center">
        ...
        <div id="acc" className="sm:col-span-3 sm:row-span-3 rounded-lg bg-white opacity-90">
          <BarChart />
        </div>
        
        <div id="popup" className="sm:col-span-3 sm:row-span-4 rounded-lg bg-white opacity-90">
        </div>
        
        <div id="tabel" className="sm:col-span-6 sm:row-span-4 rounded-lg bg-white opacity-90">
          <SensorTable sensors={sensors} /> {/* Use the SensorTable component */}
        </div>
        
        <div id="int" className="sm:col-span-3 sm:row-span-3 rounded-lg bg-white opacity-90">
        </div>
      </div>
    </div>
  );
